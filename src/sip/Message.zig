...
/// Generates a new response from incoming request.
pub fn newResponse(original: Message, gpa: std.mem.Allocator, io: std.Io, status: headers.StatusCode) !Message {
    ...
    if (original.to) |to| {
        new_message.to = try to.clone(allocator);

        //TODO This should only be set under some circumnstances, check which, some 100 and >= 200
        if (new_message.to.?.tag == null) {
            try new_message.to.?.generateTag(allocator, io);
        }
    }
    if (original.call_id) |call_id| new_message.call_id = try allocator.dupe(u8, call_id);
    if (original.sequence) |seq| new_message.sequence = seq;
...
test "sip messages generated from a request maintains required fields" {
    const message_text = "REGISTER sip:localhost SIP/2.0\r\n" ++
        "Via: SIP/2.0/UDP 172.20.10.4:55595;rport;branch=z9hG4bKPj97wgnQ5d7IM3cfDd2QYcYf9H8hqJLxit\r\n" ++
        "Max-Forwards: 49\r\n" ++
        "From: \"Streats\" <sip:streats@localhost>;tag=Z.hw-WnzbyImNj0P.WWHJW9zhtQc1lm8\r\n" ++
    ...
    const allocator = std.testing.allocator;
    var message = try Message.parse(allocator, message_text);
    var new_message = try message.newResponse(allocator, std.testing.io, .ok);
    message.deinit();

    try std.testing.expectEqual(new_message.start_line.response.status, .ok);
    try std.testing.expectEqualStrings(new_message.via.items[0].branch, "z9hG4bKPj97wgnQ5d7IM3cfDd2QYcYf9H8hqJLxit");
    try std.testing.expectEqualStrings(new_message.from.?.tag.?, "Z.hw-WnzbyImNj0P.WWHJW9zhtQc1lm8");
    try std.testing.expectEqualStrings(new_message.to.?.contact.host, "localhost");
    try std.testing.expectEqualStrings(new_message.call_id.?, "xGAGzEIoe5SHqQnmK5W2jWsIF7kThRbn");
    try std.testing.expectEqual(new_message.sequence.?.number, 37838);
